...
try:
    ...
    pass
if _browser_proxy:
    if _browser_proxy.startswith("socks"):
        co.set_argument(f"--proxy-server={_browser_proxy}")
    else:
...
TURNSTILE_SOLVE_TIMEOUT = max(3.0, float(os.environ.get("TURNSTILE_SOLVE_TIMEOUT", "8")))
TURNSTILE_SOLVE_INTERVAL = max(0.3, float(os.environ.get("TURNSTILE_SOLVE_INTERVAL", "0.75")))
...
def _tab_score(tab) -> tuple[int, str]:
    try:
    ...
    if "x.ai" in haystack or "accounts" in haystack:
        return 3, haystack
...
def _select_working_tab():
    ...
    for tab in reversed(tabs):
        score = _tab_score(tab)
        if score > best_score:
            best_score = score
            best_tab = tab

    if best_tab is not None and best_score[0] > 0:
        return best_tab
...
def start_browser():
    # 每轮从全新浏览器开始，使用独立临时 profile 目录和随机调试端口。
    ...
    browser = Chromium(co)
    page = _select_working_tab()
    return browser, page


def stop_browser():
    # 完整关闭浏览器实例，清理临时 profile 和所有孤儿 chrome 进程。
    global browser, page, _chrome_temp_dir
    temp_dir = _chrome_temp_dir
    ...
    page = None
    time.sleep(1)
    # 1) 杀属于当前 temp_dir 的 chrome 子进程
    if temp_dir:
        try:
            subprocess.run(["pkill", "-9", "-f", temp_dir], capture_output=True, timeout=5)
        except Exception:
            pass
    # 2) 清理所有孤儿 chrome 进程（本脚本是容器内唯一浏览器任务，安全清理）
    for proc_name in ("chrome", "chrome_crashpad"):
        try:
            result = subprocess.run(["pgrep", "-x", proc_name], capture_output=True, text=True, timeout=3)
    ...
    for d in _glob.glob(tempfile.gettempdir() + "/chrome_run_*"):
        if os.path.isdir(d):
...
def restart_browser():
    # 每轮完整重启浏览器，避免 Chrome 子进程泄漏导致僵尸进程堆积。
    stop_browser()
    time.sleep(1)
    start_browser()
...
def open_signup_page():
    ...
    while time.time() < deadline:
        try:
            if has_profile_form() or has_email_form():
                return
            if click_email_signup_button(timeout=2):
                return
...
const givenInput = Array.from(document.querySelectorAll('input[data-testid="givenName"], input[name="givenName"], input[autocomplete="given-name"]')).find((node) => {
    return isVisible(node) && !node.disabled && !node.readOnly;
}) || null;
const familyInput = Array.from(document.querySelectorAll('input[data-testid="familyName"], input[name="familyName"], input[autocomplete="family-name"]')).find((node) => {
    return isVisible(node) && !node.disabled && !node.readOnly;
}) || null;
const passwordInput = Array.from(document.querySelectorAll('input[data-testid="password"], input[name="password"], input[type="password"]')).find((node) => {
    return isVisible(node) && !node.disabled && !node.readOnly;
}) || null;
...
def has_email_form():
    # 如果邮箱输入框已经出现，说明入口页已进入下一步，不必强制再点一次按钮。
    refresh_active_page()
    try:
        return bool(page.run_js(
            """
...
return 'filled';
            ...
            refresh_active_page()
            if has_email_form() or has_profile_form():
                time.sleep(0.5)
            continue
...
return true;
            ...
            if clicked:
                # Wait for page to respond and check for error messages
                time.sleep(2)
                page_error = page.run_js(
                    """
const errorNodes = document.querySelectorAll('[role="alert"], [class*="error"], [class*="Error"]');
const texts = [];
...
const rejectPatterns = ['已被拒绝', 'has been rejected', 'blocked', 'not allowed'];
for (const pat of rejectPatterns) {
    if (allText.includes(pat)) {
        const idx = allText.indexOf(pat);
        texts.push(allText.substring(Math.max(0, idx - 50), idx + 80).trim());
    }
}
return texts.join(' | ');
                    """
                )
                if page_error and any(kw in page_error for kw in ['拒绝', 'rejected', 'blocked', 'not allowed']):
                    raise Exception(f"邮箱域名被拒绝: {page_error}")
...
def getTurnstileToken(timeout=TURNSTILE_SOLVE_TIMEOUT):
...
Object.defineProperty(MouseEvent.prototype, 'screenY', { value: screenY });
            ...
            if attempt == 1 or attempt % 3 == 0:
                print(f"[Turnstile] 第 {attempt} 次尝试失败: {error}")
        time.sleep(TURNSTILE_SOLVE_INTERVAL)

    raise TimeoutError(f"Turnstile 超时（{timeout:.1f}s）")
...
return value ? 'ready' : 'pending';
        ...
        if turnstile_state == "pending" and not turnstile_token:
            ...
            try:
                ...
                print(f"[Warn] Turnstile 处理超时: {error}")
                time.sleep(0.5)
                continue
...
def _cleanup_on_signal(signum, frame):
    print(f"[Signal] 收到信号 {signum}，正在清理...")
    stop_browser()
    sys.exit(128 + signum)
