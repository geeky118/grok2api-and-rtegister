...
CLOUDMAIL_ADMIN_EMAIL = str(_conf.get("cloudmail_admin_email") or _conf.get("temp_mail_admin_email") or "")
_cloudmail_api_token: str = ""
...
def get_oai_code(dev_token: str, email: str, timeout: int = 30) -> Optional[str]:
    ...
    """
    code = wait_for_verification_code(mail_token=dev_token, timeout=timeout, to_email=email)
    if code:
        code = code.replace("-", "")
    return code
...
def _detect_mail_provider(api_base: str) -> str:
    if TEMP_MAIL_PROVIDER in {"cloudmail", "cloud_mail", "cloud-mail"}:
        return "cloudmail"
...
def _provider_label() -> str:
    p = _detect_mail_provider(TEMP_MAIL_API_BASE)
    if p == "cloudmail":
        return "Cloud Mail"
    return "DuckMail" if p == "duckmail" else "Temp Mail"
...
def _get_cloudmail_token() -> str:
    """Get or refresh Cloud Mail API token via genToken endpoint."""
    global _cloudmail_api_token
    if _cloudmail_api_token:
        return _cloudmail_api_token

    api_base = TEMP_MAIL_API_BASE.rstrip("/")
    admin_email = CLOUDMAIL_ADMIN_EMAIL
    if not admin_email:
        admin_email = f"admin@{TEMP_MAIL_DOMAIN}"
    ...
    res = _do_request(
        ...
        json={"email": admin_email, "password": TEMP_MAIL_ADMIN_PASSWORD},
        timeout=20,
    )
    if res.status_code != 200:
        raise Exception(f"Cloud Mail genToken failed: {res.status_code} - {res.text[:200]}")
    ...
    if isinstance(data, dict):
        ...
        if isinstance(d, dict):
            ...
            token = d
    if not token:
        raise Exception(f"Cloud Mail genToken returned no token: {data}")

    _cloudmail_api_token = token
    return token


def _create_cloudmail_email() -> "Tuple[str, str, str]":
    """Cloud Mail: generate random address (catch-all), return (email, password, api_token)."""
    if not TEMP_MAIL_DOMAIN:
        raise Exception("temp_mail_domain is required for Cloud Mail")

    email_local = _generate_local_part(random.randint(8, 12))
    email = f"{email_local}@{TEMP_MAIL_DOMAIN}"
    api_token = _get_cloudmail_token()
    print(f"[*] Cloud Mail catch-all address: {email}")
    return email, "", api_token


def _fetch_cloudmail_emails(mail_token: str, to_email: str = "") -> "List[Dict[str, Any]]":
    """Query Cloud Mail emailList API."""
    api_base = TEMP_MAIL_API_BASE.rstrip("/")
    session, use_cffi = _create_session()
    body: "Dict[str, Any]" = {"num": 1, "size": 10, "timeSort": "desc"}
    if to_email:
        body["toEmail"] = to_email
    res = _do_request(
        session,
        use_cffi,
        "post",
        ...
        json=body,
        headers={"Authorization": mail_token, "Content-Type": "application/json"},
        timeout=20,
    )
    if res.status_code != 200:
        return []
...
def fetch_emails(mail_token: str, to_email: str = "") -> List[Dict[str, Any]]:
    """获取邮件列表。"""
    if _detect_mail_provider(TEMP_MAIL_API_BASE) == "cloudmail":
        try:
            return _fetch_cloudmail_emails(mail_token, to_email)
        except Exception:
            return []
...
def wait_for_verification_code(mail_token: str, timeout: int = 120, to_email: str = "") -> Optional[str]:
    """轮询临时邮箱，等待验证码邮件。"""
    start = time.time()
    seen_ids = set()

    while time.time() - start < timeout:
        messages = fetch_emails(mail_token, to_email=to_email)
        for msg in messages:
            if not isinstance(msg, dict):
                continue
            msg_id = msg.get("id") or msg.get("emailId")
            if not msg_id or msg_id in seen_ids:
                continue
            seen_ids.add(msg_id)

            if _detect_mail_provider(TEMP_MAIL_API_BASE) == "cloudmail":
                mail_content = msg.get("content") or msg.get("subject") or ""
                if msg.get("subject"):
                    mail_content = f"Subject: {msg['subject']}\n{mail_content}"
                code = extract_verification_code(mail_content)
            else:
                detail = fetch_email_detail(mail_token, str(msg_id))
                if not detail:
                    continue
                mail_content = _extract_mail_content(detail)
                code = extract_verification_code(mail_content)
            if code:
